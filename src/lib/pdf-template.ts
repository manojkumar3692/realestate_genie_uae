...
export function buildComputedReportData(
  ...
  const unitComparisons = bundle.unitTypes.map((unit) => {
    ...
    const exit = computeExitLiquidity(unit, bundle.financials, projection);
...
export function renderReportHtml(
  ...
  const yearlyRows = computed.focusProjection.rows
    .map(
      (r) => `
      <tr>
        <td>Year ${r.year}</td>
  ...
  const barChartSvg = barChart(
    computed.focusProjection.rows.map((r) => ({ label: `Yr ${r.year}`, value: r.propertyValue })),
    { theme, valuePrefix: "" }
  );
...
<body>
  ...
  <section class="page cover" style="${heroStyle}">
    ...
    <div class="cover-mid">
      <div class="eyebrow">Exclusive Investment Opportunity</div>
  ...
  <section class="page">
    ...
    <div class="stat-grid">
      ...
      <div class="mini-stat"><div class="mini-stat-label">Projected ${financials.projectionYears}-Yr ROI</div><div class="mini-stat-value pos">${computed.focusProjection.rows[computed.focusProjection.rows.length - 1].roiOnInvestmentPercent.toFixed(0)}%</div></div>
      <div class="mini-stat"><div class="mini-stat-label">Gross Rental Yield</div><div class="mini-stat-value">${financials.rentalYieldPercent}%</div></div>
  ...
  <section class="page">
    ...
    <p class="lede">Projected value growth for <b>${escapeHtml(computed.focusUnit.typeLabel)}</b> over ${financials.projectionYears} years, assuming ${financials.annualAppreciationPercent}% annual appreciation and ${financials.rentalYieldPercent}% gross rental yield.</p>
    <div class="chart-wrap">${barChartSvg}</div>
    <table class="table small">
      <thead><tr><th>Year</th><th>Property Value</th><th>Appreciation Gain</th><th>Annual Rent</th><th>Net Cashflow</th><th>Cumulative ROI</th></tr></thead>
  ...
  <section class="page">
    ${header(firm, theme)}
    <h2 class="section-title">Exit Strategy & Liquidity Plan</h2>
    <p class="lede">Illustrative exit scenario if the investor sells in <b>Year ${computed.focusExit.exitYear}</b>.</p>
    <div class="exit-grid">
    ...
    <p class="muted small">Investors are not obligated to exit at Year ${computed.focusExit.exitYear} — this is one illustrative liquidity scenario. Secondary market resale, refinancing, or continued rental hold are all available strategies depending on market conditions at the time.</p>
    ${footer(firm, 7)}
  </section>
